...
function isMultipleFilterValue(
  value: FilterValue
): value is MultipleFilterValue {
...
const MainCategory = styled.div`
  ...
  button {
    line-height: 1;
    padding: ${(props) =>
      `${props.theme.spaces.s150} ${props.theme.spaces.s100}`};
  }
`;
...
function ActionListDropdownInput<Value extends FilterValue>(
  props: ActionListDropdownProps<Value>
) {
  const {
    id,
    label,
  ...
  return (
    <SelectDropdown<
      SelectDropdownOption,
      Value extends MultipleFilterValue ? true : false
    >
      label={label}
      id={`${id}-field`}
      name={id}
...
function ActionListFilterBadges({
  ...
  const badges = badgesToCreate
    .map((item: ActionListFilter) => {
      ...
      return (isSingleFilterValue(value) ? [value] : value).map((v) =>
        createBadge(item, v)
      );
    })
    .flat()
  ...
  return (
    <FiltersList aria-live="assertive">
      ...
      {badges.length > 0 && (
        <span className="visually-hidden">{t('active-filters')}</span>
      )}
...
type FilterChangeCallback<Value> = (
  filterId: string,
  val: Value,
  debounce?: number
) => void;
...
abstract class DefaultFilter<Value extends FilterValue>
...
type QueryFilterCategoryType =
  ActionListCategoryTypeFilterBlock['categoryType'];
type QueryFilterCategory =
  NonNullable<QueryFilterCategoryType>['categories'][0];
type FilterCategoryType = QueryFilterCategoryType &
  CategoryTypeHierarchy<FilterCategory>;
type FilterCategory = QueryFilterCategory &
  CategoryHierarchyMember<FilterCategoryType>;
...
class CategoryFilter extends DefaultFilter<FilterValue> {
  ...
  constructor(
    ...
    const hierarchyCt = constructCatHierarchy<
      FilterCategory,
      FilterCategoryType
    >([this.ct])[0];
    ...
    const getLabel = (cat: ActionListCategory) =>
      this.ct.hideCategoryIdentifiers
        ? cat.name
        : `${cat.identifier}. ${cat.name}`;
  ...
  render(
    value: FilterValue,
    onChange: FilterChangeCallback<FilterValue>,
    t: TFunction
  ) {
      ...
      <Col sm={12} md={12} lg={12} key={this.id}>
        <MainCategory>
          <MainCategoryLabel id={`label-${this.id}`}>
            {this.getLabel(t)}
            {this.ct.helpText && (
              <PopoverTip header="Main Category" content={this.ct.helpText} />
            )}
          </MainCategoryLabel>
...
ActionListFilters.constructFilters = (opts: ConstructFiltersOpts) => {
  ...
  if (primaryFilters?.length)
    sections.push(makeSection('primary', false, primaryFilters));
  if (mainFilters?.length)
    sections.push(makeSection('main', false, mainFilters));
  if (advancedFilters?.length)
    sections.push(makeSection('advanced', true, advancedFilters));
