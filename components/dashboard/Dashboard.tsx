...
const DashboardTab = ({ t, segment }) => {
  ...
  if (segment.actions.length) {
    ...
    content = (
      <div className="mb-5 pd-5">{t('impact-group-no-actions', { context: plan.generalContent.actionTerm })}</div>
    );
  }
...
export default function Dashboard(props) {
  ...
  if (error) {
    ...
    return <p>{ t('error-loading-actions', { context: plan.generalContent.actionTerm }) }</p>;
  }
