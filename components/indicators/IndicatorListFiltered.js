...
const filterIndicators = (
  ...
  return sortIndicators(hierarchy, filteredIndicators, displayMunicipality);
};
...
const IndicatorListFiltered = (props) => {
  ...
  const allIndicatorsHaveSameLevel =
    new Set(filteredIndicators.flat().map((i) => i.level)).size === 1;
  ...
  if (flatten(filteredIndicators).length === 0) {
    return (
            ...
            <React.Fragment key={`indicator-group-${idx}`}>
              ...
              >
                ...
                {group.map((item) => {
                  ...
                  const visible =
                    filteredIndicators.length > 1 || !hierarchyEnabled
                      ? true
...
IndicatorListFiltered.propTypes = {
  ...
  filters: PropTypes.shape({
    // Filter keys are generated by ActionList.tsx and hardcoded
    // here for now while the refactor of indicators is ongoing
    'cat-theme': PropTypes.number,
