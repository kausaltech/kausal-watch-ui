...
class IndicatorListFilters extends React.Component {
  ...
  getDepth(cats, catId, depth) {
    ...
    if (currentCat.parent)
      currentDepth = this.getDepth(cats, currentCat.parent.id, depth + 1);
    return currentDepth;
  }
  ...
  render() {
    ...
    const { showIdentifiers, filterIndicatorsByAllLevels } =
      theme.settings.categories;

    let sortedCats = cloneDeep(
      cats.sort((a, b) => a.identifier.localeCompare(b.identifier))
    );
    if (!filterIndicatorsByAllLevels)
      sortedCats = sortedCats.filter((cat) => !cat.parent);
