...
function ActionPage() {
  ...
  const plan = usePlan();
  const { loading, error, data } = useQuery<GetActionDetailsQuery>(ActionContent.query, {
    variables: {
      id,
  ...
  if (error) component = <ErrorMessage message={error.message} />;
    ...
    component = <ErrorMessage statusCode={404} message={t('action-not-found', getActionTermContext(plan))} />;
  } else {
    component = <ActionContent action={data.action} extraPlanData={data.plan} />
  }
