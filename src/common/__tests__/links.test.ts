...
describe('prependPlanAndLocale', () => {
  it('prepends non-primary locales', () => {
    expect(prependPlanAndLocale(mockPlan, '/foo/bar', 'de')).toBe('/de/foo/bar');
    expect(prependPlanAndLocale(mockPlan, '', 'en-US')).toBe('/en-US/');
    expect(prependPlanAndLocale(mockPlan, 'fi/peruna', 'fi')).toBe('/fi/peruna');
  });
  ...
  it('prepends plan base paths', () => {
    expect(prependPlanAndLocale(mockPlanWithBase, '/hello/there', 'en')).toBe('/base/hello/there');
    expect(prependPlanAndLocale(mockPlanWithBase, '/', 'en')).toBe('/base/');
    expect(prependPlanAndLocale(mockPlanWithBase, '/base', 'en')).toBe('/base/');
  });
  ...
  it('prepends plan base paths and non-primary locales', () => {
    ...
    expect(prependPlanAndLocale(mockPlanWithBase, '/fi/base/', 'fi')).toBe('/fi/base/');
  });
});
