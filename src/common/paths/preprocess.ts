...
export const getMetricValue: (node: { metric: NodeMetric }, year: number) => number | undefined = (
  node,
  year
) =>
...
export const summarizeYearlyValues = (yearlyValues) => sum(yearlyValues.map((v) => v.value));
