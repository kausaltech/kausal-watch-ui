...
function ActionContentBlock(props: ActionContentBlockProps) {
  ...
  switch (block.__typename) {
    case 'ActionDependenciesBlock':
      if (!action.dependencyRole || !action.allDependencyRelationships?.length) {
        return null;
      }
    ...
    case 'ActionDescriptionBlock':
      ...
      return <ActionDescriptionBlock content={action.description} fieldLabel={block.fieldLabel} />;
    case 'ActionLeadParagraphBlock':
      if (!action.leadParagraph) return null;
      return (
        <ActionLeadParagraphBlock content={action.leadParagraph} fieldLabel={block.fieldLabel} />
      );
    ...
    case 'ActionRelatedIndicatorsBlock':
      ...
      return (
        <ActionRelatedIndicatorsBlock actionId={action.id} indicators={action.relatedIndicators} />
      );
    case 'ActionTasksBlock':
      if (!action.tasks.length) return null;
      return <ActionTasksBlock tasks={action.tasks} heading={block.fieldLabel} />;
    ...
    case 'ActionContentCategoryTypeBlock': {
      ...
      return <CategoryTags categories={categories} types={[block.categoryType]} />;
    }
...
function ActionContentProgressContainer({ action }: Pick<ActionContentProps, 'action'>) {
  const plan = usePlan();
  const t = useTranslations();
  ...
  return (
    <>
      ...
      {hasReason && (
        <StatusBadge {...commonStatusBadgeProps} reason={action.manualStatusReason ?? undefined} />
      )}
    </>
  );
}
...
function ActionContent(props: ActionContentProps) {
  ...
  const showIndicatorCausalChainBlock =
    action?.relatedIndicators.length > 0 && detailsCombined?.some(isIndicatorCausalChainBlock);
  ...
  return (
    <div data-testid={testId}>
      ...
      <StyledContentGrid>
        ...
        <StyledMainBottom>{makeComponents('detailsMainBottom')}</StyledMainBottom>
        ...
        <Container>
          <Row>
            <Col sm="12" className="mb-5">
              <ActionPager nextAction={action.nextAction} previousAction={action.previousAction} />
            </Col>
          </Row>
        </Container>
      )}
