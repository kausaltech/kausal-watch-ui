...
const StyledPhaseIndicatorContainer = styled.div<{ $isVertical: boolean }>`
  ...
  ${({ $isVertical }) => ($isVertical ? verticalIndicatorStyles : horizontalIndicatorStyles)}
`;
...
function getIconFromType(type: PhaseType, phaseIdentifier: string, isContinuous: boolean = false) {
...
function PhaseIndicator({
  ...
  return (
    <StyledPhaseIndicatorContainer $isVertical={isVertical}>
      {layout === 'horizontal' && <StyledPhaseLine $hidden={index === 0} $color={color} />}
      <Icon
    ...
    <>
      <StyledContainer ref={ref} $isVertical={isVertical} $isMini={isMini}>
        {phases.map((phase, index) => {
          ...
          const nextPhaseType =
            index + 1 < phases.length ? getPhaseType(index + 1, activePhaseIndex) : undefined;
          const phaseName =
            isContinuous && phase.identifier === 'completed' ? t('action-continuous') : phase.name;
