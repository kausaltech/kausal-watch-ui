...
export const TasksTooltipContent = ({ action, plan }: TooltipWithPlanProps) => {
  ...
  if (taskCounts.total < 1)
    return (
      <div>
        <TooltipTitle>{t('action-no-tasks', getActionTaskTermContext(plan))}</TooltipTitle>
      </div>
    );

  return (
    <TaskTooltip>
      <TooltipTitle>{t('action-tasks', getActionTaskTermContext(plan))} </TooltipTitle>
      <table>
        <tbody>
...
export const ImplementationPhaseTooltipContent = ({ action, plan }: TooltipWithPlanProps) => {
  ...
  if (!activePhase || status?.identifier === ActionStatusSummaryIdentifier.Cancelled) {
...
export const ResponsiblePartiesTooltipContent = ({ action, plan }: TooltipWithPlanProps) => {
  const t = useTranslations();
  ...
  if (parties.length < 1)
    return (
      <div>
        <TooltipTitle>{t('responsible-parties', { context: organizationTerm })}</TooltipTitle>
      </div>
    );
