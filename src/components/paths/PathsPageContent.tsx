...
const PathsPageContent = ({ page }: { page: GeneralPlanPage; pageSectionColor?: string }) => {
  ...
  return (
    <>
      ...
      {isStaticPage && (
        <ContentArea>
          <MainContent>{page.body && <StreamField page={page} blocks={page.body} />}</MainContent>
        </ContentArea>
      )}
      {isActionListPage && (
        <ContentArea>
      ...
      {!isActionListPage && !isCategoryPage && !isStaticPage && <div>Unknown page type</div>}
    </>
  );
};
