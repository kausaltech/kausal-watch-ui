...
import { GetInstanceContextQuery, InstanceGoalEntry } from '@/common/__generated__/paths/graphql';
  ...
  const instanceGoals: AugmentedInstanceGoal[] | undefined = pathsInstance?.goals.map((goal) => {
    const separateYears =
      goal.id === 'net_emissions/emission_scope:indirect'
        ...
        : null;
    const colorAdjust = goal.id === 'net_emissions/emission_scope:indirect' ? 1.75 : 0;
    const hideForecast = goal.id === 'net_emissions/emission_scope:indirect' ? true : false;
    return {
