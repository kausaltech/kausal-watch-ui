...
describe('stripLocaleAndPlan', () => {
  ...
  it('strips the passed locale', () => {
    ...
    expect(stripLocaleAndPlan(mockPlan, 'es-US', 'es-US/foo/bar')).toBe('foo/bar');
  });
  ...
  it('strips the plan', () => {
    expect(stripLocaleAndPlan(mockPlanWithBase, 'en', `/fi/${BASE_PATH}/froyo/yoyo`)).toBe(
      'fi/froyo/yoyo'
    );
    expect(stripLocaleAndPlan(mockPlanWithBase, 'en', `/${BASE_PATH}/ice-cream`)).toBe('ice-cream');
    expect(stripLocaleAndPlan(mockPlanWithBase, 'en', `/${BASE_PATH}`)).toBe('');
  });

  it('strips both the plan and locale', () => {
    expect(stripLocaleAndPlan(mockPlanWithBase, 'fi', `/fi/${BASE_PATH}/peruna/salaatti`)).toBe(
      'peruna/salaatti'
    );
    expect(stripLocaleAndPlan(mockPlanWithBase, 'de-CH', `/de-CH/${BASE_PATH}/`)).toBe('');
  });
});
